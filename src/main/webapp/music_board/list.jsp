...
<%
	...
	try{
		...
		String sqlTotal ="SELECT COUNT(*) AS total FROM music_board m INNER JOIN user u ON m.uno = u.uno WHERE open = 'O'";
		...
		String sql = "SELECT mno, title, nickname, date_format(m.rdate, '%Y-%m-%d') as rdate, hit FROM music_board m INNER JOIN user u ON m.uno = u.uno WHERE open = 'O' "; 
...
%>
                <tr onclick="location.href='view.jsp?mno=<%= mno %>'">
                    <td><%= mno %></td>
                    <td><%= title %></td>
                    <td><%= nickname %></td>
                    <td><%= rdate %></td>
