...
<%
	...
	try{
		...
		String sqlTotal ="SELECT COUNT(*) AS total FROM notice_board n INNER JOIN user u ON n.uno = u.uno WHERE open = 'O'";
		...
		String sql = "SELECT nno, title, nickname, date_format(n.rdate, '%Y-%m-%d') as rdate, hit, authorization FROM notice_board n INNER JOIN user u ON n.uno = u.uno WHERE open = 'O' "; 
...
%>
                <tr onclick="location.href='view.jsp?nno=<%= nno %>'">
                    <td><%= nno %></td>
                    <td><%= title %></td>
                    <td><%= rdate %></td>
                    <td><%= hit %></td>
